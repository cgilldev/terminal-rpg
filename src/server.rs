...
use crate::{
    ...
    ui::{
        DisplayProfile, InputDecoder, apply_game_intent, capped_area, draw_game,
        intent_allowed_at_size,
    },
};
...
use russh::{
    ...
    keys::{PrivateKey, load_secret_key, ssh_key},
...
use tokio::sync::{
    Mutex,
    mpsc::{Sender, channel as mpsc_channel, error::TrySendError},
};
...
const SSH_CURSOR_SHOW: &[u8] = b"\x1b[?25h";
...
impl TerminalHandle {
    fn start(handle: server::Handle, channel: ChannelId) -> Self {
    ...
    fn from_sender(sender: Sender<Vec<u8>>) -> Self {
        Self {
            sender,
            sink: Vec::new(),
        }
...
impl GameServer {
    fn new(display: DisplayProfile) -> Self {
        Self {
...
impl Handler for GameServer {
    ...
    async fn channel_open_session(
        ...
        let mut game = ExplorationGame::new(None)?;
        game.start();
...
pub async fn serve(options: ServeOptions) -> Result<()> {
    ...
    GameServer::new(options.display)
        .run_on_address(Arc::new(config), options.listen)
        .await?;
    Ok(())
}

fn load_or_create_host_key(path: &PathBuf) -> Result<PrivateKey> {
    if path.exists() {
